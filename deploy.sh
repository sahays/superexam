...
if [ "$UPGRADE_ONLY" = true ]; then
    ...
    gcloud run deploy superexam-website \
      ...
      --timeout=30m \
      --allow-unauthenticated \
      --set-env-vars "PROCESSING_SERVICE_URL=$PROCESSING_SERVICE_URL,GCS_BUCKET_NAME=$GCS_BUCKET_NAME,GEMINI_API_KEY=$GEMINI_API_KEY,FIRESTORE_COLLECTION_PREFIX=$FIRESTORE_COLLECTION_PREFIX,GCP_PROJECT_ID=$PROJECT_ID,GEMINI_MODEL=$GEMINI_MODEL,JWT_SECRET=$JWT_SECRET,ADMIN_CODE=$ADMIN_CODE" || { echo "Website deployment failed"; exit 1; }
fi
